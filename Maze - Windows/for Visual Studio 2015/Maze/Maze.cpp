...
int main()
{
	...
	scanf_s("%d", &Algorithm_Mode);

	printf("是否需要显示搜索迷宫路径的过程？\t【否 - 0   是 - 1】\n\t");
	scanf_s("%d", &Show_Search_Process);

	printf("是否直接使用默认设置并运行？\t【否 - 0   是 - 1】\n\t");
	int defaultset; scanf_s("%d", &defaultset);
	if (!defaultset)
		UserInput();
...
void UserInput()
{
	...
	scanf_s("%d", &Show_Generate_Process);
